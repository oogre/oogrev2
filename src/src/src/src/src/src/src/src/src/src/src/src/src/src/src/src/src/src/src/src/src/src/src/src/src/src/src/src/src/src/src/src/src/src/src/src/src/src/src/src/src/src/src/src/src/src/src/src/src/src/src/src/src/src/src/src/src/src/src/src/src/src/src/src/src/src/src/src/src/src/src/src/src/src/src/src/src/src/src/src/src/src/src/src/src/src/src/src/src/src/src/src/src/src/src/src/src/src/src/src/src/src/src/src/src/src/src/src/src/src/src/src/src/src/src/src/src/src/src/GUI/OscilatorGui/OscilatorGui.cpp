...
void OscilatorGui::setup(ofxGuiGroup * oscillatorGroup, ofTexture * tex, ofxMasterSlaveControl * msc){
    ...
    ofxGuiGroup* groupOsc = group->addGroup("Oscillator"+ofToString(_id), ofJson({
        {"show-header", false},
    }));
    groupOsc->addLabel("Oscillator : ");
    ...
    groupRotation->addLabel("Rotations : ");
